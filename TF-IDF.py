...
documents = fields.map(lambda x: x[3].split(" "))  # extract the body of the text
...
hashingTF = HashingTF(100000)  #100K hash buckets just to save some memory Hash every words to numbers
tf = hashingTF.transform(documents) # take list of words to hash values
...
#this reduces the size as it turns into a vector
